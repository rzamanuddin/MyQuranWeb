...
{
    public class ListTafsirModel : PageModelCustom
    {
        public IList<Ayah> Ayahs { get; set; } = new List<Ayah>();
        public IList<Surah> Surahs { get; set; } = new List<Surah>();

        public bool IsMobileDevice { get; set; } = false;

        public ListTafsirModel(IUnitOfWork unitOfWork)
        {
        ...
        public async Task OnGetAsync()
        {
            try
            {
                ...
                if (!IsMobileDevice)
                {
                    var surahs = (await unitOfWork.Surahs.GetAll()).ToList();
                    int division = 38;
                    for (int r = 1; r <= division; r++)
                    {
                        int i = r - 1;
                        while (i < surahs.Count)
                        {
                            Surahs.Add(surahs[i]);
                            i += division;
                        }
                ...
                {
                    Surahs = (await unitOfWork.Surahs.GetAll()).ToList();
                }
            }
            catch (Exception ex)
            {
