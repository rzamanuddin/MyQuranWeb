...
{
    public class ListJuzModel : PageModelCustom
    {
        public IList<JuzHeader> JuzHeaders { get; set; } = new List<JuzHeader>();
        public  bool IsMobileDevice { get; set; } = false;

        public ListJuzModel(IUnitOfWork unitOfWork)
        {
        ...
        public async Task OnGetAsync()
        {
            try
            {
                ...
                if (!IsMobileDevice)
                {
                    var juzs = (await unitOfWork.JuzHeaders.GetAll()).ToList();
                    int division = 15;
                    for (int r = 1; r <= division; r++)
                    {
                        int i = r - 1;
                        while (i < juzs.Count)
                        {
                            JuzHeaders.Add(juzs[i]);
                            i += division;
                        }
                    }
                }
                ...
                {
                    JuzHeaders = (await unitOfWork.JuzHeaders.GetAll()).ToList();
                }
            }
            catch (Exception ex)
            {
